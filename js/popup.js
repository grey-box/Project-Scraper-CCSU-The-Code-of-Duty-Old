...
chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
  ...
  urlform.addEventListener("submit", (event) => {
    ...
    let converToBase64 = async () => {
      for (let i = 0; i < imgLinks.length-2; i++) { //18
    ...
    const scrape = async (url) => {
      try {
        ...
        console.log(base64_array); //total-2
        ...
        for(let k=0;k<base64_array.length;k++){
            await replace(base64_array[k],k)
        }

        // base64_array.forEach((each, index) => { //18
